    ...
    <header className="h-12 bg-white dark:bg-[#1a1a1a] border-b border-slate-200 dark:border-[#282828] text-slate-800 dark:text-slate-200 px-3 sm:px-4 flex items-center justify-between select-none z-30 relative transition-colors">
      ...
      <div className="flex items-center gap-2 sm:gap-3 shrink-0">
        ...
        >
          ...
          <span className="font-bold text-xs sm:text-sm tracking-tight text-slate-900 dark:text-white">
            <span className="hidden sm:inline">LeetCode </span><span className="text-[#ffa116] text-xs font-mono font-normal">算法岗</span>
          </span>
        </div>

        <div className="h-4 w-[1px] bg-slate-200 dark:bg-[#333] hidden md:block"></div>

        {/* View Switchers - Desktop Only (Mobile uses bottom nav) */}
      ...
      <div className="flex items-center gap-1 sm:gap-2 shrink-0">
        <button
          ...
          title="上一题"
          className="p-1 sm:p-1.5 rounded hover:bg-slate-100 dark:hover:bg-[#282828] text-slate-500 hover:text-slate-900 dark:hover:text-white transition"
        >
        ...
        <div className="relative" ref={dropdownRef}>
          <button
            ...
            className="flex items-center gap-1 px-2 py-1 text-[11px] sm:text-xs rounded-md bg-slate-100 dark:bg-[#282828] hover:bg-slate-200 dark:hover:bg-[#333] transition font-medium max-w-[110px] xs:max-w-[150px] sm:max-w-[240px] truncate"
          >
            <span className="truncate">
              {currentQuestion ? `${currentIndex + 1}. ${currentQuestion.title.replace(/^【选择题】/, '')}` : '选择题目'}
            </span>
          </button>
              ...
              <div className="fixed inset-x-3 top-13 sm:inset-auto sm:top-full sm:mt-1.5 sm:left-1/2 sm:-translate-x-1/2 sm:w-80 max-h-[70vh] sm:max-h-96 bg-white dark:bg-[#262626] border border-slate-200 dark:border-[#3a3a3a] rounded-xl shadow-2xl overflow-hidden flex flex-col z-50 animate-fadeIn">
                <div className="p-2 border-b border-slate-200 dark:border-[#3a3a3a]">
                  <div className="relative">
                    ...
                    <input
                      ...
                      placeholder="快速跳转题目..."
                      className="w-full pl-8 pr-3 py-1.5 text-xs bg-slate-100 dark:bg-[#1a1a1a] rounded-lg border-0 focus:outline-none text-slate-800 dark:text-slate-200"
                      autoFocus
                    />
                  </div>
                </div>
                ...
                <div className="flex-1 overflow-y-auto p-1 text-xs custom-scrollbar">
                  {filteredDropdownList.slice(0, 50).map(q => (
                    ...
                    >
                      <span className="truncate">{q.title.replace(/^【选择题】/, '')}</span>
                      <span
      ...
      <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
        ...
        <div className="flex items-center gap-1 px-1.5 sm:px-2 py-0.5 rounded-md bg-slate-100 dark:bg-[#262626] border border-slate-200 dark:border-[#333] text-[11px] sm:text-xs font-mono">
          <button
            onClick={toggleTimer}
          ...
          <button
            onClick={resetTimer}
            className="text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 ml-0.5 hidden xs:inline-block"
            title="重置秒表"
          >
        ...
        <button
          ...
          className="p-1 sm:p-1.5 rounded text-slate-500 hover:text-slate-800 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-[#282828] transition"
        ...
        <div className="w-6 h-6 rounded-full bg-gradient-to-tr from-[#00b8a3] to-teal-400 text-slate-950 hidden sm:flex items-center justify-center font-bold text-[10px]">
          AC
        </div>
      </div>
    </header>
